...
struct AriTestParams
{
    ...
    AriTestParams(int features, int parts, int objects, int clusters, float tol = 1e-5)
        : n_features(features), n_parts(parts), n_objs(objects), k(clusters), tolerance(tol) {}
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        ...
        // Large inputs, skipped
        AriTestParams(10000, 500, 9, 9)
        // AriTestParams(100, 20, 1000, 10),  //  slow to run as a unit test
        // AriTestParams(200, 20, 300, 10),   // slow to run as a unit test
        // AriTestParams(1000, 10, 300, 10)  // slow to run as a unit test
        ),
