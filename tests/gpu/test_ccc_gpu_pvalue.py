...
def test_ccc_gpu_vs_cpu_pvalue_comparison(
    ...
    assert gpu_ccc.shape == cpu_ccc.shape, (
        f"CCC shapes don't match: GPU {gpu_ccc.shape} vs CPU {cpu_ccc.shape}"
    )
    assert gpu_pvalues.shape == cpu_pvalues.shape, (
        f"P-value shapes don't match: GPU {gpu_pvalues.shape} vs CPU {cpu_pvalues.shape}"
    )

    # Expected number of comparisons for symmetric matrix
    expected_comparisons = n_features * (n_features - 1) // 2
    assert len(gpu_ccc) == expected_comparisons, (
        f"Expected {expected_comparisons} comparisons, got {len(gpu_ccc)}"
    )
    assert len(gpu_pvalues) == expected_comparisons, (
        f"Expected {expected_comparisons} p-values, got {len(gpu_pvalues)}"
    )

    # Validate p-values are in [0, 1] range
    assert np.all((gpu_pvalues >= 0) & (gpu_pvalues <= 1)), (
        "GPU p-values should be in [0, 1]"
    )
    assert np.all((cpu_pvalues >= 0) & (cpu_pvalues <= 1)), (
        "CPU p-values should be in [0, 1]"
    )
    ...
    if np.any(valid_mask):
        ...
        if len(gpu_valid) > 1:
            ...
            assert correlation > correlation_threshold, (
                f"P-value correlation {correlation:.3f} is too low (should be > {correlation_threshold})"
            )

        print("P-value comparison stats:")
        ...
        if not np.isnan(correlation):
            ...
            print("  Correlation: N/A (single value)")
        ...
        if is_edge_case:
            ...
            min_close_percentage = (
                0.0  # Allow any difference for single comparison with few permutations
            )
        ...
        if min_close_percentage > 0:
            assert close_percentage >= min_close_percentage, (
                f"Only {close_percentage:.1%} of p-values are close (should be ≥{min_close_percentage:.0%})"
            )

        # Maximum difference shouldn't be too extreme
        assert max_diff < max_diff_threshold, (
            f"Maximum p-value difference {max_diff:.4f} is too large (should be < {max_diff_threshold})"
        )
